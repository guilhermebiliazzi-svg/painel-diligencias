...
export default function UsuariosClient({
  ...
  const [state, convidar, enviando] = useActionState<AcaoState, FormData>(
    convidarUsuario,
    undefined
  );
    ...
    <div className="mt-6 space-y-6">
      ...
      <section style={{ backgroundColor: '#ffffff' }} className="rounded-2xl border border-slate-200 p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">
          A pessoa recebe um e-mail de acesso e entra com o Google (mesmo e-mail).
        </p>
        <form action={convidar} className="mt-4 space-y-3">
          ...
          {state?.ok && !state?.aviso && (
            <p className="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-800">Convite enviado.</p>
          )}
          <button
            type="submit"
            disabled={enviando}
          ...
          return (
            <div key={u.email} style={{ backgroundColor: '#ffffff' }} className="rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <form action={salvarUsuario} className="mt-4">
                ...
                {/* self: mantém ativo+admin marcados e travados p/ evitar lockout */}
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                  ...
                  {TELAS.map(([key, label]) => (
                    <Checkbox key={key} name={key} defaultChecked={u[key as TelaKey]}>{label}</Checkbox>
                  ))}
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button type="submit" className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                    Salvar
                  </button>
                </div>
              </form>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <form action={reenviarConvite}>
                  <input type="hidden" name="email" value={u.email} />
                  <button type="submit" className="rounded-lg px-3 py-1.5 text-sm font-medium text-slate-500 transition hover:bg-slate-50 hover:text-slate-800">
                    Reenviar convite
                  </button>
                </form>
                {!souEu && (
                  <form action={removerUsuario}>
                    <input type="hidden" name="email" value={u.email} />
                    <button type="submit" className="rounded-lg px-3 py-1.5 text-sm font-medium text-red-600 transition hover:bg-red-50">
                      Remover
                    </button>
                  </form>
                )}
